...
import { Heart, MessageCircle, Send, X, Loader2, Share2, Bookmark, MoreHorizontal, Repeat2, Link2, ChevronDown, ChevronUp } from "lucide-react";
...
export function PostDetailDialog({
  ...
  if (imgAspect === 'tall') {
    return <Dialog open={!!post} onOpenChange={open => !open && onClose()}>
        <DialogContent className="max-w-[100vw] w-[100vw] h-[100vh] max-h-[100vh] p-0 overflow-hidden bg-transparent border-0 rounded-none [&>button]:hidden">
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            {/* Close button */}
            <button onClick={onClose} className="absolute top-4 left-4 z-50 p-2 rounded-full bg-black/50 hover:bg-black/70 text-white transition-colors">
              <X className="h-6 w-6" />
            </button>

            {/* Main Image - Scrollable for tall images */}
            <div className="relative z-10 w-full h-full overflow-y-auto flex justify-center">
              <OptimizedImage src={post.image_url} variants={{
            ...
            }} alt={post.title} variant="fullscreen" className="w-auto max-w-full" priority />
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-6">
              <div className="max-w-4xl mx-auto">
                ...
                {/* Artist */}
                <Link to={`/profile/${post.user_id}`} onClick={onClose} className="flex items-center gap-2 mb-4 hover:opacity-80">
                  ...
                  {post.artist_profile?.is_verified && <Badge className="h-4 px-1 text-[10px] bg-blue-500 text-white border-0">✓</Badge>}
                </Link>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    {/* Like */}
                    <button onClick={e => onLike(post.id, post.is_liked || false, e)} className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Save */}
                    <button onClick={() => isSaved ? onUnsavePost(post.original_post_id || post.id) : onSavePost(post.original_post_id || post.id)} className={`p-2 rounded-full transition-colors ${isSaved ? 'bg-primary text-white' : 'bg-white/10 hover:bg-white/20 text-white'}`}>
      ...
      <DialogContent className="max-w-[100vw] w-[100vw] h-[100vh] max-h-[100vh] p-0 overflow-hidden gap-0 border-0 rounded-none bg-transparent [&>button]:hidden">
        <div className="flex flex-col lg:flex-row h-full w-full">
          ...
          <div className="flex-1 flex items-center justify-center min-h-[40vh] lg:min-h-0 lg:h-full overflow-hidden relative">
            ...
            medium: post.image_medium_url || undefined,
            large: post.image_large_url || undefined
          }} alt={post.title} variant="fullscreen" className="relative z-10 max-h-full max-w-full" priority />
          </div>
          ...
          <div className="w-full lg:w-[420px] xl:w-[480px] flex flex-col bg-background border-l border-border max-h-[55vh] lg:max-h-full lg:h-full shrink-0">
            ...
            <div className="p-4 border-b border-border shrink-0">
              <div className="flex items-center justify-between">
                <Link to={`/profile/${post.user_id}`} onClick={onClose} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                ...
                {user && user.id !== post.user_id && <Button variant={isFollowing ? "outline" : "default"} size="sm" className="rounded-full" onClick={e => onFollow(post.user_id, isFollowing, e)}>
            ...
            <div className="p-4 border-b border-border shrink-0">
              ...
              {post.description && <p className="text-muted-foreground mt-2 text-sm leading-relaxed">
                  {renderTextWithMentions(post.description)}
                </p>}
              
              {/* Tools & Tags */}
              {(post.tools_used && post.tools_used.length > 0 || post.hashtags && post.hashtags.length > 0) && <div className="flex flex-wrap gap-1.5 mt-3">
                  {post.tools_used?.map(tool => <Badge key={`tool-${tool}`} variant="outline" className="text-xs">
                      ✦ {tool}
                    </Badge>)}
                  {post.hashtags?.map(tag => <button key={`tag-${tag}`} onClick={() => {
                onTagSelect(tag);
                onClose();
            ...
            <div className="p-3 border-b border-border flex items-center justify-between shrink-0">
              <div className="flex items-center gap-1">
                {/* Like */}
                <button onClick={e => onLike(post.id, post.is_liked || false, e)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full hover:bg-muted transition-colors">
              ...
              <div className="flex items-center gap-0.5">
                {/* Repost */}
                <button onClick={() => onShareDialogOpen(post)} className={`p-2 rounded-full hover:bg-muted transition-colors ${isReposted ? 'text-green-500' : 'text-muted-foreground hover:text-foreground'}`} title="Repost">
                  <Repeat2 className="h-5 w-5" />
                </button>
                {/* Save */}
                <button onClick={() => isSaved ? onUnsavePost(post.original_post_id || post.id) : onSavePost(post.original_post_id || post.id)} className={`p-2 rounded-full hover:bg-muted transition-colors ${isSaved ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`} title="บันทึก">
                  <Bookmark className={`h-5 w-5 ${isSaved ? 'fill-current' : ''}`} />
                </button>
                {/* Share dropdown */}
                <DropdownMenu>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-secondary">
              {commentsLoading ? <div className="flex justify-center py-4">
                  <Loader2 className="h-6 w-6 animate-spin" />
                </div> : comments.length === 0 ? <p className="text-center text-muted-foreground py-8 text-sm">
                  ยังไม่มีความคิดเห็น
                </p> : comments.map(comment => <div key={comment.id} className="space-y-3">
                    {/* Parent Comment */}
                    <div className="group flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        {editingComment?.id === comment.id ? <div className="space-y-2">
                            <Textarea value={editCommentContent} onChange={e => onEditCommentContentChange(e.target.value)} className="min-h-[60px] text-sm" autoFocus />
                            ...
                            <p className="text-sm">
                              ...
                              {comment.artist_profile?.is_verified && <Badge className="h-4 px-1 text-[10px] bg-blue-500 text-white border-0 mr-1">✓</Badge>}
                              {isBuyerUser(comment.artist_profile) && <Badge className="h-4 px-1.5 text-[10px] bg-sky-500 text-white border-0 mr-2">Buyer</Badge>}
                              {renderTextWithMentions(comment.content)}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-muted-foreground">{formatTimeAgo(comment.created_at)}</span>
                              {user && <button onClick={() => onStartReply(comment)} className="text-xs text-muted-foreground hover:text-foreground font-medium">
                                  ตอบกลับ
                                </button>}
                              {user && user.id === comment.user_id && <button onClick={() => onStartEditComment(comment)} className="text-xs text-muted-foreground hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                                  แก้ไข
                                </button>}
                              {user && (user.id === comment.user_id || user.id === post.user_id) && <button onClick={() => onDeleteComment(comment.id)} disabled={deletingCommentId === comment.id} className="text-xs text-destructive hover:text-destructive/80 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {comment.replies && comment.replies.length > 0 && <>
                        {!expandedReplies.has(comment.id) ? <button onClick={() => onToggleReplies(comment.id, true)} className="ml-11 text-xs text-primary hover:underline flex items-center gap-1">
                            <ChevronDown className="h-3 w-3" />
                            ดูการตอบกลับ {comment.replies.length} รายการ
                          </button> : <div className="ml-8 pl-3 border-l-2 border-muted space-y-3">
                            <button onClick={() => onToggleReplies(comment.id, false)} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1">
                              <ChevronUp className="h-3 w-3" />
                              ซ่อนการตอบกลับ
                            </button>
                            {comment.replies.map(reply => <div key={reply.id} className="group flex gap-2">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm">
                                    ...
                                    {reply.artist_profile?.is_verified && <Badge className="h-3 px-1 text-[8px] bg-blue-500 text-white border-0 mr-1">✓</Badge>}
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {user && (user.id === reply.user_id || user.id === post.user_id) && <button onClick={() => onDeleteComment(reply.id)} disabled={deletingCommentId === reply.id} className="text-[10px] text-destructive opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {/* Reply Input */}
                    {replyingToComment?.id === comment.id && user && <div className="ml-8 pl-3 border-l-2 border-primary/30">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <input type="text" value={replyContent} onChange={e => onReplyContentChange(e.target.value)} onKeyDown={e => {
                    if (e.key === 'Enter' && !e.shiftKey && replyContent.trim() && !submittingReply) {
                      e.preventDefault();
                      onSubmitReply(comment);
                    }
                  }} placeholder="เขียนการตอบกลับ..." className="flex-1 bg-background border border-border rounded-full px-4 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" autoFocus disabled={submittingReply} />
            ...
            {/* Comment Input */}
            {user && <div className="p-4 border-t border-border shrink-0">
                <div className="flex gap-2 items-center">
                  <Avatar className="h-8 w-8 shrink-0">
                    <AvatarImage src={user?.user_metadata?.avatar_url || undefined} />
                    <AvatarFallback className="text-xs">
                  ...
                  <div className="flex-1">
                    <MentionInput value={newComment} onChange={onNewCommentChange} placeholder="เขียนความคิดเห็น... พิมพ์ @ เพื่อแท็กผู้ใช้" rows={1} className="min-h-[40px] resize-none rounded-full" />
                  </div>
                  <Button size="icon" className="shrink-0 rounded-full" onClick={onSubmitComment} disabled={!newComment.trim() || submittingComment}>
